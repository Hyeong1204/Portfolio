...
public class ItemData_EquipItem : ItemData, IEquipItem
{
    ...
    /// <param name="slot">아이템이 들어잇는 인벤토리 슬롯</param>
    ...
    public virtual void AutoEquipItem(GameObject target, ItemSlot slot)
    {
        ...
        if (equipTarget != null)
        {
            ...
            if (partsSlot != null)
            {
                ...
                if (equipItem != this)
                {
                    EquipItem(target, slot);          // 같은 파츠인데 다른 아이템이 장비 시도 되었으면 다른 아이템을 장비
                }
            }
            else
            {
                // 지금 장비된 아이템이 없다.
                EquipItem(target, slot);              // 장비된 아이템 장착
            }
        }
    }
}
