...
public class UserPlayer : PlayerBase
{
    ...
    Ship SelectedShip
    {
        ...
        set
        {
            ...
            {
                if (selectedShip != null)                         // 이전 targetShip에 대한 처리

                {
                    selectedShip.SetMaterialType();               // 함선의 머티리얼 타입을 normal로 돌리기
                    selectedShip.gameObject.SetActive(false);     // 비활성화 시키기
                }
                ...
                if (selectedShip != null)                         // 새 targetShip이 있으면 그것에 대한 처리
                {
                    ...
                    SelectedShip.transform.position = board.GridToWorld(girdPos);     // 해당위치에 함선 배치
    ...
    /// <summary>
    /// 클릭 되었을 때 실행될 델리게이트들 (게임 상태별로 각각 가지고 있음, null이면 해당 상태에서 입력 받는 일이 없다는 것)
    /// </summary>
    ...
    /// <summary>
    /// 마우스의 휠이 돌아갔을 때 실행될 델리게이트들 (게임 상태별로 각각 가지고 있음, null이면 해당 상태에서 입력 받는 일이 없다는 것)
    /// </summary>
    ...
    protected override void Awake()
    {
        ...
        onClick = new Action<Vector2>[length];                  // 크기 잡기
        onMouseMove = new Action<Vector2>[length];
        onMouseWheel = new Action<float>[length];

        onClick[(int)GameState.ShipDeploymnet] = OnClick_ShipDeployment;            // 함수 등록
        onClick[(int)GameState.Battle] = OnClick_Battle;
        onMouseMove[(int)GameState.ShipDeploymnet] = OnMouseMove_ShipDeployment;
        onMouseWheel[(int)GameState.ShipDeploymnet] = OnMouseWheel_ShipDeployment;
    }
    ...
    {
        onClick[(int)state]?.Invoke(screenPos);     // 상태별로 델리게이트 실행
    }
    ...
    {
        onMouseMove[(int)state]?.Invoke(screenPos); // 상태별로 델리게이트 실행
    }
    ...
    {
        onMouseWheel[(int)state]?.Invoke(wheelDelta);// 상태별로 델리게이트 실행
    ...
    private void OnClick_ShipDeployment(Vector2 screenPos)
    {
        ...
        if (SelectedShip != null && board.ShipDeplyment(SelectedShip, world))   // 선택된 함선이 있으면 보드에 배치
        {
            ...
            SelectedShip = null;            // 선택된 함선은 null로 만들고
    ...
    private void OnMouseMove_ShipDeployment(Vector2 screenPos)
    {
        if (SelectedShip != null && !SelectedShip.IsDeployed)
        {
            ...
            SelectedShip.transform.position = board.GridToWorld(girdPos);     // 계산한 위치로 TargetShip 옮기기

            bool isSuccess = board.IsShipDeployment(SelectedShip, girdPos);   // 배치 가능한지 확인해서 머티리얼 업데이트
    ...
    private void OnMouseWheel_ShipDeployment(float wheelDelta)
    {
        ...
        if (SelectedShip != null) // SelectedShip이 있으면
        {
            SelectedShip.Rotate(ccw);     // 횔방향에 따라 회전

            bool isSuccess = board.IsShipDeployment(SelectedShip, SelectedShip.transform.position); // 배치 가능한지 확인
            ShipManager.Inst.SetDeployModeColor(isSuccess);     // 머티리얼 업데이트
        }
    }
    ...
    {
        SelectedShip = ships[(int)shipType - 1];        // shipType이 가르키는 배를선택
    }
