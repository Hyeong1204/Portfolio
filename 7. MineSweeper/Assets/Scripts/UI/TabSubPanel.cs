...
public class TabSubPanel : MonoBehaviour
{
    ...
    /// <summary>
    /// 어떤 종류의 랭킹을 표시할 것인지를 결정하는 enum 타입
    /// </summary>
    ...
    private void Awake()
    {
        rankLines = GetComponentsInChildren<RankUI>();
    }
    ...
    public void Refresh()
    {
        ...
        switch (rankType)
        {
            case RankType.Action:
                ...
                {
                    rankLines[index].SetRankAndRecord<int>(index + 1, data);
                    index++;
                }
                break;
            case RankType.Time:
                ...
                {
                    rankLines[index].SetRankAndRecord<float>(index + 1, data);
                    index++;
                }
                break;
            default:
