...
public class RankPanel : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            // 탭이 선택 되었을 때 실행될 델리게이트에 람다 함수 등록
            tab.onTabSelect += (newSelectedTab) =>
            {
                ...
                toggle.SetToggleState(true);            // 탭이 눌러지면 무조건 토글 버튼
            };
        }

        // 자식 토글 버튼 찾기
        toggle = GetComponentInChildren<ToggleButton>();
        // 토글 버튼 on / off 될 때 실행될 델리게이트에 람다 함수 등록
        toggle.onToggleChange += (isOn) =>
        {
            if (isOn && selectedTab != null)            // 토글 버튼이 켜지고 선택된 탭, 선택된 탭 체크 이유 : 토글 버튼의 start가 RanPanel의 Start보다 먼저 실행 되었을 때 문제 발생
            {
