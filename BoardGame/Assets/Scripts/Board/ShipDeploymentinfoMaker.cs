...
public class ShipDeploymentinfoMaker : MonoBehaviour
{
    ...
    private void Awake()
    {
        // 딕셔너리 생성 (키 : 함선타입, 값 : 함선 종류별 표시용 오브젝트)
        infoObjects = new Dictionary<ShipType, List<GameObject>>(ShipManager.Inst.ShipTpyeCount);
    ...
    {
        foreach (var pos in positions)      // 모든 위치를 숨회하면서
        {
