...
public class GameManager : Singleton<GameManager>
{
    ...
    /// <summary>
    /// 게임이 종료 되었음을 확인할 수 있는 프로퍼티
    /// </summary>
    public bool IsGameEnd => userPlayer.IsDepeat || EnemyPlayer.IsDepeat;

    // 입력 ----------------------------------------------------------------------------------------------
    InputContorller input;

    public InputContorller Input => input;
    ...
    protected override void ManagerDataReset()
    {
        ...
        onStateChange = null;       // 씬이 새로 로딩이 되면 이전에 연결 되어있떤 플레이어 함수들은 사용할 수 없게 되므로 초기화 처리
        if (userPlayer != null)
        {
    ...
    {
        base.Initialize();
        input = GetComponent<InputContorller>();
    }
